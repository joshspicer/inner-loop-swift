        ...
        if (process.env.ENABLE_LLM === 'true' && process.env.AUTO_ANALYZE_ERRORS === 'true') {
            ...
            setImmediate(async () => {
                try {
                    ...
                    // Could store analysis or send notifications here
                } catch (err) {
                    console.error('Error analyzing with LLM:', err.message);
                }
            });
        }
        ...
        if (process.env.ENABLE_LLM === 'true' && batchData.userMessage) {
            ...
            setImmediate(async () => {
                try {
                    ...
                    console.error('Error analyzing batch with LLM:', err.message);
                }
            });
        }
