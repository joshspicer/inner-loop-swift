...
import { ErrorData, BatchData, LogEntry } from './plugin-system';
...
export class LLMProvider {
  ...
  async analyzeError(errorData: ErrorData): Promise<string> {
    const prompt = this.buildErrorPrompt(errorData);
    return await this.callLLM(prompt);
  }

  async analyzeBatch(batchData: BatchData): Promise<string> {
    const prompt = this.buildBatchPrompt(batchData);
    return await this.callLLM(prompt);
  }

  private buildErrorPrompt(errorData: ErrorData): string {
...
- Total Logs: ${batchData.logs.length}
      ...
      const req = https.request(options, (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            reject(new Error('Failed to parse Anthropic response: ' + (error as Error).message));
          }
        });
      });
